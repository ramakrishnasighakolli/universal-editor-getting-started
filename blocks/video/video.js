...
const loadVideoWithConsent = (videoContainer, link, autoplay, showModal) => {
  ...
  if ((isYoutube || isVimeo) && typeof OtsukaPCM !== 'undefined') {
    ...
    // Direct video file or consent not available, load normally
  ...
  if (placeholder) {
    ...
    if (!autoplay) {
      ...
      wrapper.addEventListener('click', () => {
        wrapper.remove();
        loadVideoWithConsent(videoContainer, link, true, true);
      });
    }
